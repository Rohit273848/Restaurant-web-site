import{s as e,$,c as M}from"./main-DfgMMrCK.js";function g(t,a="$"){return typeof t!="number"?`${a}0.00`:`${a}${t.toFixed(2)}`}function j(t=5){const a=Math.floor(t),r=t%1>=.5;let o="";for(let s=0;s<5;s++)s<a?o+='<svg class="w-3.5 h-3.5 text-brand-gold fill-current" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/></svg>':s===a&&r?o+='<svg class="w-3.5 h-3.5 text-brand-gold fill-current opacity-70" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/></svg>':o+='<svg class="w-3.5 h-3.5 text-slate-600 fill-current" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/></svg>';return`<div class="flex items-center gap-1">${o} <span class="text-xs font-bold text-slate-300 ml-1">${t.toFixed(1)}</span></div>`}function y(t=0){if(!t||t<=0)return"";let a="";for(let r=0;r<t;r++)a+=`<span title="Spice Level ${t}">🌶️</span>`;return`<span class="text-xs flex items-center gap-0.5 ml-2" aria-label="Spice level ${t} of 3">${a}</span>`}function V(t,a="$"){if(!t)return"";const r=typeof a=="string"?{currencySymbol:a,variant:"default"}:{currencySymbol:"$",variant:"default",showBadges:!0,ctaText:"Details",...a},{id:o,title:s="Untitled Dish",description:b="",price:i=0,image:p="",tags:v=[],dietary:f={},rating:x=4.9,spiceLevel:d=0,isAvailable:L=!0,availability:z="in-stock",categoryName:S=""}=t,n=!L||z==="sold-out",m=r.currencySymbol||"$",h=p||"https://images.unsplash.com/photo-1544025162-d76694265947?auto=format&fit=crop&w=800&q=80";let u="";r.showBadges!==!1&&(v.includes("chef-special")?u+=`<span class="px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider bg-brand-gold/90 text-slate-950 rounded-full shadow-sm">Chef's Special</span>`:(v.includes("popular")||v.includes("best-seller"))&&(u+='<span class="px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider bg-amber-500/90 text-slate-950 rounded-full shadow-sm">Popular</span>'));let c="";f.isVegan?c+='<span class="text-xs px-2 py-0.5 rounded bg-emerald-950/80 border border-emerald-500/30 text-emerald-400 font-medium">Vegan</span>':f.isVegetarian&&(c+='<span class="text-xs px-2 py-0.5 rounded bg-emerald-950/80 border border-emerald-500/30 text-emerald-300 font-medium">Vegetarian</span>'),f.isGlutenFree&&(c+='<span class="text-xs px-2 py-0.5 rounded bg-amber-950/80 border border-amber-500/30 text-amber-300 font-medium">GF</span>');const w=n?'<div class="absolute inset-0 bg-slate-950/80 backdrop-blur-xs flex items-center justify-center z-20"><span class="px-3 py-1.5 bg-red-950 border border-red-500/50 text-red-300 font-bold text-xs uppercase tracking-widest rounded-md">Sold Out</span></div>':"";return r.variant==="compact"?`
    ...
    `:r.variant==="horizontal"?`
      <article class="food-card food-card-horizontal flex flex-col sm:flex-row bg-brand-dark-card border border-brand-dark-border rounded-xl overflow-hidden shadow-dark-card hover:border-brand-gold/60 transition-all cursor-pointer group relative ${n?"opacity-60":""}" data-dish-id="${e(o)}">
        <div class="relative sm:w-56 shrink-0 aspect-[4/3] sm:aspect-auto bg-slate-900">
          ...
          <div class="absolute top-3 left-3 flex flex-wrap gap-1 z-10">${u}</div>
          ${w}
        </div>
        <div class="p-5 flex-1 flex flex-col justify-between">
          ...
          <div class="pt-3 border-t border-brand-dark-border/60 flex items-center justify-between">
            <div class="flex items-center gap-2">${c} ${y(d)}</div>
    ...
    `:r.variant==="featured"?`
      <article class="food-card food-card-featured grid grid-cols-1 lg:grid-cols-12 gap-6 p-6 bg-brand-dark-card border border-brand-gold/40 rounded-2xl shadow-2xl shadow-gold-glow/10 group cursor-pointer relative ${n?"opacity-60":""}" data-dish-id="${e(o)}">
        <div class="lg:col-span-7 relative rounded-xl overflow-hidden aspect-[16/10] bg-slate-900">
          ...
          <div class="absolute top-4 left-4 z-10 flex gap-2">${u}</div>
    ...
    `:r.variant==="premium"?`
      <article class="food-card food-card-premium relative bg-gradient-to-b from-brand-gold/20 via-brand-dark-card to-brand-dark-card border border-brand-gold/50 rounded-2xl overflow-hidden shadow-gold-glow/20 group cursor-pointer flex flex-col ${n?"opacity-60":""}" data-dish-id="${e(o)}">
        <div class="relative overflow-hidden aspect-[4/3] bg-slate-900">
          ...
          <div class="absolute top-3 left-3 z-10 flex gap-1">${u}</div>
        ...
        <div class="p-6 flex-1 flex flex-col justify-between">
          ...
          <div class="pt-3 border-t border-brand-dark-border/60 flex items-center justify-between">
            <div class="flex items-center gap-1.5">${c}</div>
  ...
  `}let l=null;function B(){l||(l=M(`
    ...
    `),document.body.appendChild(l),$("#modal-close-btn",l).addEventListener("click",k),l.addEventListener("click",a=>{a.target===l&&k()}),window.addEventListener("keydown",a=>{a.key==="Escape"&&l.classList.contains("active")&&k()}))}function T(t,a="$"){B();const{title:r,description:o,price:s,image:b,tags:i=[],dietary:p={}}=t,v=$("#modal-body",l),f=$(".modal-content",l);let x="";i.includes("chef-special")&&(x+=`<span class="px-3 py-1 text-xs font-bold bg-brand-gold text-slate-950 rounded-full">Chef's Signature</span>`),i.includes("popular")&&(x+='<span class="px-3 py-1 text-xs font-bold bg-amber-500 text-slate-950 rounded-full">Popular Favorite</span>');let d="";p.isVegan&&(d+='<span class="px-2.5 py-1 text-xs font-medium rounded bg-emerald-950 border border-emerald-500/40 text-emerald-300">100% Vegan</span>'),p.isVegetarian&&(d+='<span class="px-2.5 py-1 text-xs font-medium rounded bg-emerald-950 border border-emerald-500/40 text-emerald-300">Vegetarian</span>'),p.isGlutenFree&&(d+='<span class="px-2.5 py-1 text-xs font-medium rounded bg-amber-950 border border-amber-500/40 text-amber-300">Gluten Free</span>'),p.containsNuts&&(d+='<span class="px-2.5 py-1 text-xs font-medium rounded bg-rose-950 border border-rose-500/40 text-rose-300">Contains Nuts</span>'),v.innerHTML=`
  ...
  `,l.classList.add("active"),l.setAttribute("aria-hidden","false"),document.body.classList.add("overflow-hidden"),requestAnimationFrame(()=>{f.classList.remove("scale-95","opacity-0"),f.classList.add("scale-100","opacity-100")})}function k(){if(!l)return;const t=$(".modal-content",l);t.classList.remove("scale-100","opacity-100"),t.classList.add("scale-95","opacity-0"),setTimeout(()=>{l.classList.remove("active"),l.setAttribute("aria-hidden","true"),document.body.classList.remove("overflow-hidden")},250)}export{T as o,V as r};
