...
export function renderFoodCard(dish, optionsOrCurrency = '$') {
  ...
  if (opts.variant === 'compact') {
    return `
      <article class="food-card food-card-compact flex items-center gap-4 p-3 bg-brand-dark-card border border-brand-dark-border rounded-xl shadow-dark-card hover:border-brand-gold/50 transition-all cursor-pointer group relative ${isSoldOut ? 'opacity-60' : ''}" data-dish-id="${sanitize(id)}">
        ...
        <div class="flex-1 min-w-0">
          <h4 class="font-serif text-base font-bold text-slate-100 group-hover:text-brand-gold transition-colors truncate mb-0.5">${sanitize(title)}</h4>
          <span class="text-xs font-serif font-bold text-brand-gold">${formatCurrency(price, currencySymbol)}</span>
        </div>
  ...
  if (opts.variant === 'horizontal') {
    return `
      <article class="food-card food-card-horizontal flex flex-col sm:flex-row bg-brand-dark-card border border-brand-dark-border rounded-xl overflow-hidden shadow-dark-card hover:border-brand-gold/60 transition-all cursor-pointer group relative ${isSoldOut ? 'opacity-60' : ''}" data-dish-id="${sanitize(id)}">
        ...
        <div class="p-5 flex-1 flex flex-col justify-between">
          <div>
            <div class="flex items-start justify-between gap-3 mb-2">
              <h3 class="font-serif text-xl font-bold text-slate-100 group-hover:text-brand-gold transition-colors">${sanitize(title)}</h3>
              <span class="px-3 py-1 bg-slate-950/90 border border-brand-gold/40 rounded-full text-sm font-bold text-brand-gold font-serif shrink-0">${formatCurrency(price, currencySymbol)}</span>
            </div>
            <p class="text-sm text-slate-400 line-clamp-2 leading-relaxed mb-4">${sanitize(description)}</p>
          </div>
          <div class="pt-3 border-t border-brand-dark-border/60 flex items-center justify-between">
            ...
            <span class="inline-flex items-center gap-1 text-xs font-semibold text-brand-gold group-hover:translate-x-1 transition-transform">${opts.ctaText || 'Details'} &rarr;</span>
          </div>
        </div>
      </article>
    `;
  ...
  if (opts.variant === 'featured') {
    return `
      <article class="food-card food-card-featured grid grid-cols-1 lg:grid-cols-12 gap-6 p-6 bg-brand-dark-card border border-brand-gold/40 rounded-2xl shadow-2xl shadow-gold-glow/10 group cursor-pointer relative ${isSoldOut ? 'opacity-60' : ''}" data-dish-id="${sanitize(id)}">
        <div class="lg:col-span-7 relative rounded-xl overflow-hidden aspect-[16/10] bg-slate-900">
          ...
          <div class="absolute bottom-4 right-4 z-10 px-4 py-1.5 bg-slate-950/90 border border-brand-gold/50 backdrop-blur-md rounded-full text-base font-bold text-brand-gold font-serif">${formatCurrency(price, currencySymbol)}</div>
          ${soldOutBadge}
        </div>
        <div class="lg:col-span-5 flex flex-col justify-between py-2">
          <div>
            <div class="mb-3">${renderRating(rating)}</div>
            <h3 class="font-serif text-2xl font-bold text-slate-100 mb-3 group-hover:text-brand-gold transition-colors">${sanitize(title)}</h3>
            <p class="text-sm text-slate-300 leading-relaxed font-light mb-6">${sanitize(description)}</p>
          </div>
  ...
  if (opts.variant === 'premium') {
    return `
      <article class="food-card food-card-premium relative bg-gradient-to-b from-brand-gold/20 via-brand-dark-card to-brand-dark-card border border-brand-gold/50 rounded-2xl overflow-hidden shadow-gold-glow/20 group cursor-pointer flex flex-col ${isSoldOut ? 'opacity-60' : ''}" data-dish-id="${sanitize(id)}">
        <div class="relative overflow-hidden aspect-[4/3] bg-slate-900">
          ...
          <div class="absolute bottom-3 right-3 z-10 px-3.5 py-1 bg-brand-gold text-slate-950 font-serif font-bold text-base rounded-full shadow-gold-glow">${formatCurrency(price, currencySymbol)}</div>
          ${soldOutBadge}
        </div>
        <div class="p-6 flex-1 flex flex-col justify-between">
          <div>
            <div class="flex items-center justify-between mb-2">
              <span class="text-[10px] uppercase font-bold tracking-widest text-brand-gold font-sans">${sanitize(categoryName || 'Signature')}</span>
              ${renderRating(rating)}
            </div>
            <h3 class="font-serif text-xl font-bold text-slate-100 group-hover:text-brand-gold transition-colors mb-2">${sanitize(title)}</h3>
            <p class="text-sm text-slate-300 line-clamp-2 leading-relaxed mb-4 font-light">${sanitize(description)}</p>
          </div>
          <div class="pt-3 border-t border-brand-dark-border/60 flex items-center justify-between">
            <div class="flex items-center gap-1.5">${dietaryHtml}</div>
            <span class="inline-flex items-center gap-1 text-xs font-semibold text-brand-gold group-hover:translate-x-1 transition-transform">${opts.ctaText || 'Pairing Details'} &rarr;</span>
          </div>
        </div>
      </article>
    `;
  ...
  // -------------------------------------------------------------
  return `
    <article class="food-card cursor-pointer group flex flex-col bg-brand-dark-card border border-brand-dark-border rounded-xl overflow-hidden shadow-dark-card hover:border-brand-gold/60 transition-all duration-300 ${isSoldOut ? 'opacity-60' : ''}" data-dish-id="${sanitize(id)}">
      <div class="relative overflow-hidden aspect-[4/3] bg-slate-900">
        <img 
